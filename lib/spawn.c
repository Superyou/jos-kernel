...
exec(const char *prog, const char **argv)
{
	...
	1. cleanup current environment's page tables and page directory entries.
	2. cleanup file descriptor table for current environment.
	2.b what will happen to trapframe of current environment???....basically restore it to new env environemt state. 
	3. Load the new program binary.
	4. setup new stack page and approriately set the esp and eip same as done in fork() while creating new enviroment.
	...
	*/
	...
	//1.cleanup current enviroments page tables ans page directory entries for this process(thisenv).
	// sys_cleanup will also restore the trapframe to initial state
	sys_env_cleanup(0);

	//2.cleanup file descriptor table for current environment.
	// we dont have to take special care for this. as fd structure pages with PTE_SHARE bit enabled which will be cleaned up above only
